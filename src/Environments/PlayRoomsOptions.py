...
from PlayRooms import PlayRooms

class PlayRoomsOptions( ):
    ...
    def create( spec, K=1, scheme = 'none', count = 20, *args ):
        ...
        return OptionEnvironment( PlayRoomsOptions, env.S, env.A, env.P, env.R, env.R_bias, env.start_set, env.end_set, O )

    @staticmethod
    def reset_rewards( env, spec, K=1, *args ):
        O = env.O
        env = PlayRooms.reset_rewards( env, spec, K )
        return OptionEnvironment( PlayRoomsOptions, env.S, env.A, env.P, env.R, env.R_bias, env.start_set, env.end_set, O )
